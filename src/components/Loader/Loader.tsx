...
const LoaderSvg = styled(Loader5)`
  ...
  fill: ${(props: IStyledProps): string => props?.theme?.elementsColor || props.customTheme};
  margin: ${(props: IStyledProps): string => props.indents};
  animation: ${Spin} 1s linear 0s infinite;
`;

export const Loader: React.FC<IProps> = ({ width, theme, customTheme, indents }) => {
  return (
    <LoaderSvg
      width={width || '30px'}
      theme={theme}
      customTheme={customTheme || '#7394bf'}
      indents={indents || ''}
    />
  );
};
